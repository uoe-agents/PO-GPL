...
parser.add_argument('--target-training-steps', type=int, default=16000000, help="Training data size.")
...
class GPLOnlyModelTraining(object):
    ...
    def evaluate_demo_episode(self, action_shape, model, cg_model, true_state_directory):
        ...
        for j in range(self.config["num_eval_episodes"]):
            ...
            for k in range(1,dim_test): 
                ...
                out = model(current_memory, eval=True, analysis_mode=True)
                ...
                q_vals = cg_model(
                    out["others"]["graph_particles"], out["latent_state"], torch.ones([
                    current_memory["current_obs"].size()[0], self.config["num_particles"], current_memory["current_obs"].size()[1]
                ]).to(device).double(),"inference", action_dist, None
                )
                ...
                agent_existence_prob = out["others"]["agent_existence_prob"].reshape(1,self.config["num_particles"],-1)

                ## print(agent_existence_prob.reshape(1,self.config["num_particles"],-1))
                ## print()
                #input()
                agent_existence_dist = dist.Bernoulli(probs=agent_existence_prob)
                
                # method 1: Using sample. 
                predicted_existence_sample = agent_existence_dist.sample()
                existence_difference_sample = (predicted_existence_sample.sum(-1).mean(-1) - existence_state.flatten().sum(-1))**2

                # method 2: using the mean of the distribution
                predicted_existence_mean = agent_existence_dist.mean
                existence_difference_mean = (predicted_existence_mean.sum(-1).mean(-1) - existence_state.flatten().sum(-1))**2
                


                # # print(existence_difference_mean, existence_difference_sample)
                ...
                # existence squared 
                buff_agent_existence_squarred[num_dones, n_updates] = existence_difference_sample.detach().numpy()
                buff_agent_existence_squarred_mean[num_dones, n_updates] = existence_difference_mean.detach().numpy()
                # existence log prob 
                buff_agent_existence_log_prob[num_dones, n_updates] = out["others"]["agent_existence_log_prob"].detach().numpy()
                # action 
                ...
                # print(out["others"]["state_reconstruction_log_prob"].detach().numpy(), existence_difference_sample.detach().numpy())
    ...
    def run(self):
        ...
        try: 
            ...
            print("You should run glance first")
