...
class GPLBeliefModelState(BeliefModelPriv):
    def __init__(self,
                 action_space,
                 nr_inputs,
                 agent_inputs,
                 ...
                 with_s_sigmoid=False,
                 s_var_noise=0.0,
                 add_prev_timestep_log_weight=False,
                 with_rnn_s_processing = False
                 ):
        ...
        self.add_prev_timestep_log_weight = add_prev_timestep_log_weight
        self.with_rnn_s_processing=with_rnn_s_processing
        ...
        self.proposal_state_prediction_network = ProposalStatePredictionNetwork(
            ...
            state_hid_dims, state_message_dims, device=self.device, with_sigmoid=with_s_sigmoid,
            s_var_noise=s_var_noise
        ).to(self.device)
    ...
    def new_latent_state(self):
        ...
        initial_state = st.State(
                theta=torch.zeros(
                    self.batch_size, self.num_particles, self.num_agents, self.theta_dim
                ).to(device).double()
        )

        log_weight = torch.zeros(self.batch_size, self.num_particles).to(device).double()
        initial_state.log_weight = log_weight
        initial_state.s = torch.zeros(
            self.batch_size, self.num_particles, self.num_agents, self.s_dim
        ).to(device).double()

        # Initialize the state as if no agents exist beforehand
        initial_state.i = torch.zeros(self.batch_size, self.num_particles, self.num_agents, 1).to(device).double()

        initial_state.cell1 = torch.zeros(
            self.batch_size, self.num_particles, self.num_agents, self.theta_dim
        ).to(device).double()

        initial_state.cell2 = torch.zeros(
            self.batch_size, self.num_particles, self.num_agents, self.theta_dim
        ).to(device).double()
    ...
    def propagate(self, observation, reward, actions, previous_latent_state, other_actions, state, state_agent_existence, with_resample=True, eval=False, with_noise="False"):
        ...
        # REMOVED: resampling with ratio
        # This is my original idea. Basically if ratio is true, we implment a different resampling algorithm. 
        # # calculate variance of weights
        # variance = torch.var(old_log_weight, dim=1) 
        # # Only update if weights are higher than variance 
        # ancestral_indices = sample_ancestral_index_ratio(old_log_weight, self.resample_ratio_treshold, variance.cpu().detach().numpy())
        ...
        if self.with_rnn_s_processing:
            setattr(current_latent_state, "lstm_h", previous_latent_state.lstm_h)
            setattr(current_latent_state, "q_hids1", previous_latent_state.q_hids1)
            setattr(current_latent_state, "q_hids2", previous_latent_state.q_hids2)
        ...
        updated_agent_flags, proposed_agent_log_existence, agent_existence_logits, proposed_agent_existence_entropy, s_rep_sample, s_rep_dist, agent_state_existence_logs, s_rep_var = \
        ...
        if self.add_prev_timestep_log_weight:
            new_log_weight = new_log_weight + previous_latent_state.log_weight

        if self.with_global_feature:
            state_reconstruction_log_prob = a_state_log_prob.mean(dim=-1).mean(dim=-1) + u_state_log_prob.mean(dim=-1)
        else:
            state_reconstruction_log_prob = a_state_log_prob.mean(dim=-1).mean(dim=-1) 
        ...
        others["proposed_inv_state_rep_var"] = (1.0/s_rep_var).mean()
        others["target_inv_state_rep_var"] = (1.0/s_distrib_params[1]).mean()
        others["proposal_state_var"] = s_rep_var
        others["state_var"] = s_distrib_params[1]
...
class SRepresentationProcessing(nn.Module):
    def __init__(self,
                 s_dim,
                 h_dim,
                 device
                 ):
        super(SRepresentationProcessing, self).__init__()
        self.h_dim = h_dim
        self.device = device
        self.RNNPreprocessingModule = nn.LSTM(s_dim, h_dim, batch_first=False).to(self.device)

    def new_latent_state(self, latent_states):
        latent_state_s_size = latent_states.s.size()
        hids = (
            torch.zeros(
                latent_state_s_size[0], latent_state_s_size[1], latent_state_s_size[2], self.h_dim
            ).to(self.device).double(),
            torch.zeros(
                latent_state_s_size[0], latent_state_s_size[1], latent_state_s_size[2], self.h_dim
            ).to(self.device).double()
        )
        latent_states.lstm_h = torch.zeros(
                latent_state_s_size[0], latent_state_s_size[1], latent_state_s_size[2], self.h_dim
        ).to(self.device).double()
        latent_states.q_hids1 = hids[0]
        latent_states.q_hids2 = hids[1]
        return latent_states

    def forward(self, latent_states):
        latent_state_s_size = latent_states.s.size()
        prev_hids = (latent_states.q_hids1, latent_states.q_hids2)

        input_c1 = prev_hids[0].view(-1, prev_hids[0].size(-1)).unsqueeze(0)
        input_c2 = prev_hids[1].view(-1, prev_hids[1].size(-1)).unsqueeze(0)

        lstm_h, (new_hids_c1, new_hids_c2) = self.RNNPreprocessingModule(
            latent_states.s.view(1,-1,latent_state_s_size[-1]), (input_c1, input_c2)
        )

        lstm_h = lstm_h.view(latent_state_s_size[0], latent_state_s_size[1], latent_state_s_size[2], -1) * latent_states.i
        new_hids_c1 = new_hids_c1.view(latent_state_s_size[0], latent_state_s_size[1], latent_state_s_size[2], -1) * latent_states.i
        new_hids_c2 = new_hids_c2.view(latent_state_s_size[0], latent_state_s_size[1], latent_state_s_size[2], -1) * latent_states.i

        latent_states.lstm_h = lstm_h
        latent_states.q_hids1 = new_hids_c1
        latent_states.q_hids2 = new_hids_c2
...
class DecisionMakingModel(nn.Module):
    def __init__(self,
        ...
        if with_rnn:
            self.cg = JointUtilLayer(h_dim + theta_dim,mid_pair, mid_nodes, action_shape, mid_pair_out, device=self.device).to(self.device)
    ...
    def cg_input_preprocess(self, graph_batch, latent_states):
        ...
        if self.with_rnn:
            h = latent_states.lstm_h
        num_agents = s.size()[-2]
        node_representations = torch.cat([s, theta], dim=-1)
        if self.with_rnn:
            node_representations = torch.cat([h, theta], dim=-1)
...
class StatePredictionNetwork(nn.Module):
    ...
    def __init__(
            ...
            autoreg_m_size, device, with_sigmoid, s_var_noise=0.0
        ):
        """
            Args:
                action_encoding : The length of action encodings in the current approach.
        ...
        # using variance for var as in SWB
    ...
    def forward(self, previous_latent_state, current_latent_state, temperature=1.0):
        ...
        s_rep_mean = self.s_update_net_mean(updated_agent_rep)
        s_rep_var = self.s_update_net_var(updated_agent_rep)


        s_rep_var = s_rep_var + self.var_noise
...
class ProposalStatePredictionNetwork(nn.Module):
    ...
    def forward(self, observation_states, previous_latent_state, current_latent_state, temperature=1.0, eval=False):
        ...
        return updated_agent_flags, agent_existence_logs, all_node_logits, agent_existence_entropy, s_rep_sample, s_rep_dist, agent_state_existence_logs, s_rep_var
...
class TypeUpdateNet(nn.Module):
    ...
    def __init__(
        ...
        self.type_rnn = nn.LSTM(s_dim + action_encoding + 1, h_dim, batch_first=False)
        self.action_encoding = action_encoding
    ...
    def forward(self, observation_states, previous_latent_state, current_latent_state):
        ...
        input = torch.cat([
            ...
            cur_flag,
            agent_act
        ], -1).view(-1, s_dim + 1 + self.action_encoding)
