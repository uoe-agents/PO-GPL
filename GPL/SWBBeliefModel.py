...
class SWBBeliefModel(BeliefModel):
    def __init__(self,
                 action_space,
                 nr_inputs,
                 agent_inputs,
        ...
        self.max_memory_duration = max_memory_duration
    ...
    def new_latent_state(self):
        ...
        initial_state.recently_seen = torch.zeros(self.batch_size, self.num_particles, self.num_agents, 1).to(device).double()
        initial_state.since_last_seen = torch.zeros(self.batch_size, self.num_particles, self.num_agents, 1).to(device).double()
    ...
    def propagate(self, observation, reward, actions, previous_latent_state, other_actions,
                  state_agent_existence, eval, with_resample):
        """
        ...
        # Reset recently seen flags of agents that were last seen more than memory duration
        expired_seen_flag_indices = previous_latent_state.since_last_seen[:,:,:,0] > self.max_memory_duration
        previous_latent_state.recently_seen[expired_seen_flag_indices]= 0

        # Switch recently seen flags of agents thaat were just observed

        recently_seen_flags = torch.clamp(
            previous_latent_state.recently_seen + (current_observation.all_x[:,:,:,1] == 1).double().unsqueeze(-1),
            min=0, max=1
        )

        # Increment the since last seen counter only for agents that have been seen recently
        since_last_seen_flags = recently_seen_flags * (previous_latent_state.since_last_seen + 1)

        # Unless if agents were seen now, where we set since last seen flag to 0
        since_last_seen_flags[current_observation.all_x[:, :, :, 1] == 1] = 0

        setattr(current_latent_state, "recently_seen", recently_seen_flags)
        setattr(current_latent_state, "since_last_seen", since_last_seen_flags)
        ...
        updated_agent_flags, s_rep_sample, s_rep_dist = \
        ...
        s_distrib_params = self.state_prediction_network(
            previous_latent_state, current_latent_state
        )
        ...
        new_log_weight = state_component + \
...
class StatePredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        self.state_network = nn.Sequential(
            nn.Linear(s_dim + h_dim + 1, gnn_hdim1),
            nn.ReLU(),
    ...
    def forward(self, previous_latent_state, current_latent_state):
        """
...
class ProposalStatePredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        self.gnn = nn.Sequential(
            nn.Linear(s_dim + h_dim + action_encoding+ 1 + o_enc_dim, gnn_hdim1),
            nn.ReLU(),
            nn.Linear(gnn_hdim1, gnn_hdim2),
            nn.ReLU(),
            nn.Linear(gnn_hdim2,gnn_out_dim)
    ...
    def forward(self, observation_states, previous_latent_state, current_latent_state):
        ...
        input = torch.cat([
            ...
            input_obs,
            input_act
        ], -1).view(-1, h_dim + s_dim + self.action_encoding + 1 + input_obs_dim)

        n_out = self.gnn(input)
        reshaped_n_out = n_out.view(batch_size, num_particles, num_agent, -1)

        # Updated agent flags are set to one for agents that have been seen recently
        updated_agent_flags = current_latent_state.recently_seen
        ...
        return updated_agent_flags, s_rep_sample, s_rep_dist
...
class StateDecoderNet(nn.Module):
    ...
    def __init__(
        ...
        self.gnn = RFMBlock(
            s_dim + h_dim + 1, 0, 0,
            gnn_hdim1, gnn_hdim2, decoder_hid_dims[-1]
        )
    ...
    def forward(self, graph, current_latent_state):
        """
            Outputs the distribution that determines whether agents are visible/invisible in obs
            and the distribution over the features of agents that are visible in the obs
