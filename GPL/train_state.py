...
# Dataset sizes
parser.add_argument('--target-training-steps', type=int, default=6400000, help="Number of experiences used for training.")
...
parser.add_argument('--act-reconstruction-weight', type=float, default=0.1,
                    help="Weight associated to action prediction loss.")
...
parser.add_argument('--eval-seed', type=int, default=500, help="Seed for AHT evaluation.")
...
class ModelTraining(object):
    ...
    def compute_disagreement_distribution_entropy(self, q_values):
        ...
        counter = torch.zeros([self.config["num_collection_threads"], num_actions]).to(
            torch.device(self.config['designated_cuda'] if self.config['use_cuda'] and torch.cuda.is_available() else "cpu")
        )
    ...
    def eval_policy_performance(self, action_shape, model, cg_model, logger, logging_id, q_rnn_model=None):
        ...
        if self.config["with_rnn_s_processing"] == "True":
            initial_states = q_rnn_model.new_latent_state(initial_states)
        avgs = []
        ...
        while (any([k < self.config["num_eval_episodes"] for k in num_dones])):
            ...
            if self.config["with_rnn_s_processing"] == "True":
                out["latent_state"] = q_rnn_model(out["latent_state"])
    ...
    def run(self):
        ...
        model = GPLBeliefModelState(
            ...
            gnn_state_hid_dims=[
                self.config["state_gnn_hid_dims1"],
                self.config["state_gnn_hid_dims2"],
                self.config["state_gnn_hid_dims3"],
                self.config["state_gnn_hid_dims4"]
            ],
            ...
            with_rnn_s_processing = True if self.config["with_rnn_s_processing"]  == "True" else False
        )

        s_processing_model = None
        if self.config["with_rnn_s_processing"] == "True":
            s_processing_model = SRepresentationProcessing(self.config["s_dim"], self.config["q_rnn_dim"], device=device)
        ...
        cg_model = DecisionMakingModel(
            ...
            with_rnn=True if self.config["with_rnn_s_processing"] == "True" else False,
            h_dim=self.config["q_rnn_dim"]
        )
        ...
        if self.config["load_from_checkpoint"] == -1:
            ...
            if self.config["with_rnn_s_processing"] == "True":
                s_processing_model = s_processing_model.double()
            model_optimizer = optim.Adam(list(model.parameters()) + list(cg_model.parameters()), lr=self.config["lr"])

            if self.config["with_rnn_s_processing"] == "True":
                model_optimizer = optim.Adam(list(model.parameters()) + list(cg_model.parameters()) + list(s_processing_model.parameters()),
                                             lr=self.config["lr"])
            ...
            if self.config["with_rnn_s_processing"] == "True":
                torch.save(
                    s_processing_model.state_dict(),
                    self.config["saving_dir"] + "/" + random_experiment_name + "/" + "0-q-rnn.pt"
                )
            ...
            self.eval_policy_performance(action_shape, model, cg_model, writer, 0, s_processing_model)
            ...
            if self.config["with_rnn_s_processing"] == "True":
                s_processing_model.load_state_dict(
                    torch.load(
        ...
        for ep_id in range(start_ep_id, num_episodes):
            ...
            initial_states = model.new_latent_state()
            if self.config["with_rnn_s_processing"] == "True":
                initial_states = s_processing_model.new_latent_state(initial_states)
            ...
            total_action_prediction_entropy = 0
            total_agent_existence_entropy = 0
            total_particle_entropy = 0
            total_agent_predicted_action_log_prob = 0
            total_agent_proposed_action_log_prob = 0
            total_agent_existence_log_prob = 0
            total_proposed_agent_existence_log_prob = 0
            ...
            total_state_reconstruction_log_prob = 0
            total_action_disagreement_entropy = 0
            proposal_state_var = 0
            state_var = 0
            ...
            while idx < self.config["eps_length"]:
                ...
                idx += 1
                steps_since_log += 1
                out = model(current_memory, with_resample=True, with_noise=self.config['use_resample_noise'])
                action_dist = model.predict_action(
                    out["others"]["graph"], out["latent_state"].detach()
                )

                if self.config["with_rnn_s_processing"] == "True":
                    out["latent_state"] = s_processing_model(out["latent_state"])
                ...
                action_entropy = self.compute_disagreement_distribution_entropy(q_vals).mean()
                total_action_disagreement_entropy += action_entropy
                ...
                if self.config["no_gradient_joint_action"]=="False":
                ...
                total_loss += self.config["state_reconstruction_weight"] * -avg_state_log_prob_loss

                print(
                    "State reconstruction loss  : ",
                    self.config["state_reconstruction_weight"] * -avg_state_log_prob_loss
                )

                # Add log probability of action reconstruction to logging
                total_state_reconstruction_log_prob += avg_state_log_prob_loss

                # Add entropy losses
                action_prediction_entropy = out["others"]["proposed_action_entropy"].mean(dim=-1).mean(dim=-1).mean()
                agent_existence_entropy = out["others"]["proposed_agent_existence_entropy"].mean(dim=-1).mean(
                    dim=-1).mean()

                total_action_prediction_entropy += action_prediction_entropy
                total_agent_existence_entropy += agent_existence_entropy

                entropy_loss = -(
                    action_prediction_entropy
                )
                ...
                state_reconstruction_loss = -(out["others"]["states_reconstruction_component"].mean())
                total_loss += self.config["agent_existence_prediction_weight"] * state_reconstruction_loss
                print(
                    "State reconstruction loss : ",
                    self.config["agent_existence_prediction_weight"] * state_reconstruction_loss
                )

                print(avg_encoding_loss, avg_act_log_prob_loss, entropy_loss, state_reconstruction_loss)
                ...
                # Calculate teammate existence log prob in state
                total_state_teammate_log_prob += out["others"]["proposed_state_agent_existence"]

                # Log encoding loss components
                total_agent_predicted_action_log_prob += out["others"]["agent_predicted_action_log_prob"].mean(
                    dim=-1).mean(dim=-1).mean()
                total_agent_proposed_action_log_prob += out["others"]["agent_proposed_action_log_prob"].mean(
                    dim=-1).mean(dim=-1).mean()
                total_agent_existence_log_prob += out["others"]["agent_existence_log_prob"].mean(dim=-1).mean(
                    dim=-1).mean()
                total_proposed_agent_existence_log_prob += out["others"]["proposed_agent_existence_log_prob"].mean(
                    dim=-1).mean(dim=-1).mean()
                ...
                total_agent_features_log_prob += out["others"]["agent_features_log_prob"].mean(dim=-1).mean(
                    dim=-1).mean()
                # if not out["others"]["u_features_log_prob"] == 0:
                proposal_state_var += out["others"]["proposal_state_var"].mean(dim=-1).mean()
                state_var += out["others"]["state_var"].mean(dim=-1).mean()
                ...
                out = model(current_memory, with_resample=False, with_noise=False)
                if self.config["with_rnn_s_processing"] == "True":
                    out["latent_state"] = s_processing_model(out["latent_state"])
                ...
                total_loss += self.config["stdev_regularizer_weight"] * (
                        out["others"]["proposed_inv_state_rep_var"] + out["others"]["target_inv_state_rep_var"]
                )
                total_q_loss += self.config["q_loss_weight"] * q_loss
                print(
                    "Value loss : ",
                    self.config["q_loss_weight"] * q_loss
                )

                # If gradient truncated at every timestep (No backprop through time)
                if self.config["no_bptt"] == "True":
                ...
                if idx % self.config["update_period"] == 0:
                    ...
                    log_dict = {
                        ...
                        "total_action_prediction_entropy": (total_action_prediction_entropy + 0.0) / steps_since_log,
                        "total_agent_existence_entropy": (total_agent_existence_entropy + 0.0) / steps_since_log,
                        ...
                        "total_agent_predicted_action_log_prob": (
                                                                             total_agent_predicted_action_log_prob + 0.0) / steps_since_log,
                        "total_agent_proposed_action_log_prob": (
                                                                            total_agent_proposed_action_log_prob + 0.0) / steps_since_log,
                        "total_agent_existence_log_prob": (total_agent_existence_log_prob + 0.0) / steps_since_log,
                        "total_proposed_agent_existence_log_prob": (
                                                                               total_proposed_agent_existence_log_prob + 0.0) / steps_since_log,
                        ...
                        "total_state_reconstruction_log_prob": (total_state_reconstruction_log_prob+0.0)/steps_since_log,
                        "total_action_disagreement_entropy": (total_action_disagreement_entropy + 0.0) / steps_since_log,
                        "proposal_state_var": (proposal_state_var + 0.0) / steps_since_log,
                        "state_var": (state_var + 0.0) / steps_since_log,
                    }
