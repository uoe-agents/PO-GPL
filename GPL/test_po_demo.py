            ...
            for k in range(1,dim_test): 
                ...
                if "Foraging" in self.config["env_name"] or "Navigation" in self.config["env_name"]:
                    ...
                    visualizer.render_every_particle(state_dist.mean, # estimation of every particle
                                        ...
                                        current_memory["current_obs"][:,:,:].squeeze(0).detach().numpy(), # current obs
                                        weights,
                                        num_dones, n_updates, 
                ...
                if self.config["verbose"]:
                    print("true state")
                    print(current_memory["current_state"][:,:,:].squeeze(0).detach().numpy())
                    print("current_obs_ ")
                    print(current_memory["current_obs"][:,:,:].squeeze(0).detach().numpy())
                ...
                # existence log prob 
                buff_agent_existence_log_prob[num_dones, n_updates] = out["others"]["agent_existence_log_prob"].mean(dim=-1).mean(dim=-1).mean().detach().numpy()
                buff_agent_existence_log_prob_proposed[num_dones, n_updates] = out["others"]["proposed_agent_existence_log_prob"].mean(dim=-1).mean(dim=-1).mean().detach().numpy()
