...
import torch
import os
from RNNTypeModel import GPLTypeInferenceModelObsRecons, GPLDecisionMakingModel
...
parser.add_argument('--obs-reconstruction-weight', type=float, default=0.01, help="Weight associated to obs reconstruction loss.")
...
parser.add_argument('--cone-angle', type=float, default=math.pi/3.0, help="Obs cone angle for FAttack")
...
class GPLOnlyModelTraining(object):
    ...
    def run(self):
        ...
        model = GPLTypeInferenceModelObsRecons(
        ...
        for ep_id in range(start_ep_id, num_episodes):
            ...
            while idx < self.config["eps_length"]:
                ...
                avg_obs_log_prob_loss = out["others"]["obs_reconstruction_log_prob"]
                total_loss += self.config["obs_reconstruction_weight"] * -avg_obs_log_prob_loss
                total_loss += self.config["act_reconstruction_weight"] * -avg_act_log_prob_loss
                print(
                    "Obs prediction loss  : ",
                    self.config["obs_reconstruction_weight"] * -avg_obs_log_prob_loss
                )
                ...
                total_obs_reconstruction_log_prob += avg_obs_log_prob_loss
