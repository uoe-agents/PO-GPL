...
class PriviledgedGPLBeliefModel(BeliefModel):
    def __init__(self,
                 action_space,
                 nr_inputs,
                 agent_inputs,
...
class ProposalStatePredictionNetwork(nn.Module):
    ...
    def forward(self, observation_states, previous_latent_state, current_latent_state):
        ...
        updated_agent_flags = observation_states.all_state_existence.view(
            [batch_size, num_particles, num_agent, 1]
        ).double().to(self.device)
...
class StateDecoderNet(nn.Module):
    ...
    def __init__(
        ...
        self.device = device
        self.gnn = RFMBlock(
            s_dim + h_dim + action_encoding + 1, 0, 0,
