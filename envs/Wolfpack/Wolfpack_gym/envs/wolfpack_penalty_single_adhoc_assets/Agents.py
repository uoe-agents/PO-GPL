...
class DistilledCoopAStarAgent(Agent):
    def __init__(self, id, obs_type="full_graph",
        ...
        self.dqn_net = GraphOppoModel(6, 0, 12, 60, 40, 50, 30, 35, 5).to("cpu")
    ...
    def act(self, obs):
        node_obs = torch.Tensor(obs[0]).to("cpu")
        u_obs = torch.Tensor(obs[1])[None, :].to("cpu")
        ...
        edge_feats = torch.zeros([graph.number_of_edges(), 0]).to("cpu")
...
class DQNAgent(Agent):
    def __init__(self, agent_id, args=None, obs_type="partial_obs", obs_height=9, obs_width=17, mode="test"):
        ...
        #self.device = torch.device("cuda" if self.args['with_gpu'] and torch.cuda.is_available() else "cpu")
        self.device = torch.device("cpu")
        self.dqn_net = DQN(17, 9, 32, self.args['max_seq_length'], 7, mode="partial", device=self.device)

        self.mode = mode
        if not self.mode == "test":
            self.optimizer = optim.Adam(self.dqn_net.parameters(), lr=self.args['lr'])
            self.target_dqn_net = DQN(17, 9, 32, self.args['max_seq_length'], 7, mode="partial").to(self.device)
    ...
    def load_parameters(self, filename):
        self.dqn_net.load_state_dict(torch.load(filename, map_location=self.device))
    ...
    def act(self, obs, added_features=None, mode="train", epsilon=0.01):
        ...
        net_inp = torch.Tensor([self.recent_obs_storage.transpose([0, 3, 1, 2])]).to(self.device)
...
class GraphDQNAgent(Agent):
    ...
    def prep(self, obs, hiddens):
        ...
        if hiddens is None:
            n_hid = (torch.zeros([1, graph.number_of_nodes(), self.dim_lstm_out]).to(self.device),
                    torch.zeros([1, graph.number_of_nodes(), self.dim_lstm_out]).to(self.device))
            e_hid = (torch.zeros([1, graph.number_of_edges(), self.dim_lstm_out]).to(self.device),
                    torch.zeros([1, graph.number_of_edges(), self.dim_lstm_out]).to(self.device))
            u_hid = (torch.zeros([1, len(obs), self.dim_lstm_out]).to(self.device),
                    torch.zeros([1, len(obs), self.dim_lstm_out]).to(self.device))
        ...
        if not self.prev_graph is None:
            ...
            if graph.number_of_nodes() - n_hid[0].shape[1] > 0:
                ...
                n_hid = (torch.cat([n_hid[0], torch.zeros([1,added_nodes, self.dim_lstm_out]).to(self.device)], dim=1).to(self.device),
                         torch.cat([n_hid[1], torch.zeros([1,added_nodes, self.dim_lstm_out]).to(self.device)], dim=1).to(self.device))
                e_hid = (torch.cat([e_hid[0], torch.zeros([1,fin_num_edge - prev_num_edge, self.dim_lstm_out]).to(self.device)], dim=1).to(self.device),
                         torch.cat([e_hid[1], torch.zeros([1,fin_num_edge - prev_num_edge, self.dim_lstm_out]).to(self.device)], dim=1).to(self.device))

        return graph, e_ob.to(self.device), n_ob.to(self.device), u_ob.to(self.device), e_hid, n_hid, u_hid
