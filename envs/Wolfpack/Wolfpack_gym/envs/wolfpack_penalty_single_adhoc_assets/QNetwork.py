...
class GraphOppoModel(nn.Module):
    def __init__(self,dim_in_node, dim_in_edge, dim_in_u, hidden_dim, hidden_dim2, dim_mid, dim_out,
        ...
        self.GNBlock = RFMBlock(dim_mid + dim_in_node + dim_in_u,
                                ...
                                dim_mid).to(self.device)
        self.GNBlock2 = RFMBlock(dim_out + 2 * dim_mid, 4 * dim_mid, 2 * dim_out + dim_mid, hidden_dim2, dim_out).to(self.device)

        self.head = nn.Linear(dim_out, added_mid_dims).to(self.device)
        self.head2 = nn.Linear(added_mid_dims, act_dims).to(self.device)
