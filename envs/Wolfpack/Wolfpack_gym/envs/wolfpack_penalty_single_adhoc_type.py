...
class WolfpackPenaltySingleAdhocType(gym.Env):
    ...
    def __init__(self, grid_height=20, grid_width=20, num_players=5, max_food_num=2,
        ...
        self.disappearabce_prob = disappearance_prob
        ...
        teammate_location_box_low = [-1] * len(teammate_location_box_high)
        opponent_box_low = [-1] * len(opponent_box_high)
        ...
        print(teammate_location_box_high, teammate_location_box_low)
        types_type_box_high = [8] * self.max_player_num
        types_type_box_low = [-1] * self.max_player_num
        ...
        self.observation_space = spaces.Dict({
            ...
            'teammate_types': spaces.Box(high=np.asarray(types_type_box_high),
                                         low=np.asarray(types_type_box_low))
        })
        ...
        if self.rnn_with_gnn:
            self.observation_space = spaces.Dict({
                ...
                'oppo_actions': spaces.Box(high=np.asarray(oppo_mod_box_high),
                                           low=np.asarray(oppo_mod_box_low)),
                'oppo_actions_shuffled': spaces.Box(high=np.asarray(oppo_mod_box_high),
                                           low=np.asarray(oppo_mod_box_low)),
                'num_agents': spaces.Box(high=np.asarray([0]),
                                         low=np.asarray([self.max_player_num])),
                'teammate_location_shuffled': spaces.Box(high=np.asarray(teammate_location_box_high),
                                                         low=np.asarray(teammate_location_box_low)),
                'teammate_location_shuffled_complete': spaces.Box(high=np.asarray(teammate_location_box_high),
                                                         low=np.asarray(teammate_location_box_low)),
                'opponent_info_complete': spaces.Box(high=np.asarray(opponent_box_high),
                                            low=np.asarray(opponent_box_low)),
                'oppo_actions_shuffled_complete': spaces.Box(high=np.asarray(oppo_mod_box_high),
                                                    low=np.asarray(oppo_mod_box_low)),
                'teammate_types': spaces.Box(high=np.asarray(types_type_box_high),
                                         low=np.asarray(types_type_box_low))
            })
        ...
        self.player_disappearance = None
        self.player_perturbation = None
        ...
        self.other_agents_list, other_agents_type = self.sample_teammates(self.num_players - 1)
        #print("Initial teammates", self.other_agents_list)
        #print("Initial teammates Type", [other_agents_type]*(self.num_players - 1))
        self.other_agents_types =  [other_agents_type]*(self.num_players - 1)
        ...
        self.main_sight_radius = main_sight_radius
    ...
    def reset(self):
        ...
        self.other_agents_list, other_agents_type  = self.sample_teammates(self.init_num_players - 1)
        ...
        self.player_obs_type = [self.obs_type]
        self.player_obs_type.extend([agent.obs_type for agent in self.other_agents_list])
        self.player_disappearance = []
        for idx in range(len(self.player_positions)):
            self.player_disappearance.append(self.prng.uniform())
        self.player_disappearance[0] += self.disappearabce_prob

        self.player_perturbation = []
        for idx in range(2*len(self.player_positions)):
            self.player_perturbation.append(
                self.prng.choice([-1,1], p=[0.5,0.5]) *
                self.prng.choice(list(range(len(self.perturbation_probs))), p=self.perturbation_probs)
            )
        ...
        for idx in range(len(self.other_agents_list) + 1):
            if self.is_close_manhattan_dist(self.player_positions[idx], self.player_positions[0]) and self.prev_player_disappearance[idx] >= self.disappearabce_prob:
                remaining_flags[counter] = 1
                counter += 1

        if not self.collapsed:
            player_obses[0]["remaining_flags"] = remaining_flags

        other_agents_types_full = np.zeros(self.max_player_num) - 1
        for i in range(len(self.other_agents_types)):
            other_agents_types_full[i] = int(self.other_agents_types[i])
        
        player_obses[0]["teammate_types"] = other_agents_types_full
    ...
    def update_state(self, hunter_collective_action, food_collective_action):
        ...
        for a in self.player_positions:
            ...
            if "full_rgb" in self.obs_type:
                self.RGB_grid[a[0]][a[1]] = [0, 0, 0]
            self.RGB_padded_grid[a[0] + self.pads][a[1] + self.pads] = [0, 0, 0]
        self.prev_player_positions = copy.deepcopy(self.player_positions)
    ...
    def observation_computation(self, obs_type, agent_type="player", agent_id=0):
        ...
        if obs_type == "main_player":
            ...
            if not self.with_rgb:
                if self.with_shuffling:
                    ...
                    for idx, player_locs in enumerate(self.player_positions):
                        perturbation_x, perturbation_y = self.player_perturbation[2 * idx], self.player_perturbation[2 * idx + 1]
                        exact_loc = [idx_s for idx_s, idx_st in enumerate(self.valid_indices) if idx_st==idx][0]
                        teammate_locations[2*exact_loc] = min(max(0,player_locs[0]+perturbation_x), self.grid_height-1)
                        teammate_locations[2*exact_loc+1] = min(max(0,player_locs[1]+perturbation_y),self.grid_width-1)

                        if agent_id == 0:
                            if (not self.is_close_manhattan_dist(player_locs, self.player_positions[0])) or self.player_disappearance[idx] < self.disappearabce_prob:
                                teammate_locations[2 * exact_loc] = -1
                                teammate_locations[2 * exact_loc + 1] = -1

                else:
                    teammate_locations = []
                    for idx, player_locs in enumerate(self.player_positions):
                        perturbation_x, perturbation_y = self.player_perturbation[2*idx], self.player_perturbation[2*idx+1]
                        player_loc = (min(max(0,player_locs[0]+perturbation_x), self.grid_height-1), min(max(0,player_locs[1]+perturbation_y),self.grid_width-1))
                        if agent_id == 0:
                            if self.is_close_manhattan_dist(player_locs, self.player_positions[0]) and self.player_disappearance[idx] >= self.disappearabce_prob:
                                teammate_locations.extend(list(player_loc))
                ...
                if self.rnn_with_gnn:
                    ...
                    for idx, player_locs in enumerate(self.player_positions):
                        perturbation_x, perturbation_y = self.player_perturbation[2 * idx], self.player_perturbation[2 * idx + 1]
                        exact_loc = [idx_s for idx_s, idx_st in enumerate(self.valid_indices) if idx_st == idx][0]
                        teammate_locations_shuffled[2 * exact_loc] = min(max(0,player_locs[0]+perturbation_x), self.grid_height-1)
                        teammate_locations_shuffled[2 * exact_loc + 1] = min(max(0,player_locs[1]+perturbation_y),self.grid_width-1)

                        if agent_id == 0:
                            if (not self.is_close_manhattan_dist(player_locs, self.player_positions[0])) or self.player_disappearance[idx]< self.disappearabce_prob:
                                teammate_locations_shuffled[2 * exact_loc] = -1
                                teammate_locations_shuffled[2 * exact_loc + 1] = -1

                    complete_teammate_locations_shuffled = [-1] * (2 * self.max_player_num)
                    for idx, player_locs in enumerate(self.player_positions):
                        exact_loc = [idx_s for idx_s, idx_st in enumerate(self.valid_indices) if idx_st == idx][0]
                        complete_teammate_locations_shuffled[2 * exact_loc] = player_locs[0]
                        complete_teammate_locations_shuffled[2 * exact_loc + 1] = player_locs[1]

                    observations['teammate_location_shuffled_complete'] = np.asarray(
                        complete_teammate_locations_shuffled
                    )
                ...
                complete_food_locs = []
                for a in self.food_positions:
                    complete_food_locs.extend(list(a))

                if agent_id == 0:
                    for a in self.food_positions:
                        perturbation_x = self.prng.choice([-1,1], p=[0.5,0.5]) * self.prng.choice([0,1], p=[0.8,0.2])
                        perturbation_y = self.prng.choice([-1,1], p=[0.5,0.5]) * self.prng.choice([0,1], p=[0.8,0.2])

                        added_food_pos = (min(max(0,a[0]+perturbation_x), self.grid_height-1), min(max(0,a[1]+perturbation_y),self.grid_width-1))
                        if self.is_close_manhattan_dist(a, self.player_positions[0]):
                            food_locs.extend(list(added_food_pos))
                        else:
                            food_locs.extend([-1,-1])

                opponent_info = []
                opponent_info.extend(food_locs)

                complete_opponent_info = []
                complete_opponent_info.extend(complete_food_locs)

                for idx, orientation in enumerate(self.food_orientation):
                    or_vector = [0] * 4
                    or_vector_complete = [0] * 4
                    if agent_id == 0:
                        if self.is_close_manhattan_dist(self.food_positions[idx], self.player_positions[0]):
                            or_vector[orientation] = 1
                    else:
                        or_vector[orientation] = 1
                    or_vector_complete[orientation] = 1
                    opponent_info.extend(or_vector)
                    complete_opponent_info.extend(or_vector_complete)

                observations['teammate_location'] = np.asarray(teammate_locations)
                observations['opponent_info'] = np.asarray(opponent_info)
                if self.rnn_with_gnn:
                    observations['opponent_info_complete'] = np.asarray(complete_opponent_info)
            ...
            # Change : Must also create action shuffling
            if self.with_oppo_mod:
                oppo_actions = []
                other_acts = None
                if self.other_player_acts is None:
                    ...
                    other_act = copy.deepcopy(self.other_player_acts)
                    other_act = [int(a) for a in other_act]
                    other_acts = []
                    if agent_id == 0:
                        for idx, player_loc in enumerate(self.prev_player_positions[1:]):
                            if self.is_close_manhattan_dist(player_loc, self.prev_player_positions[0]) and self.prev_player_disappearance[idx+1] >= self.disappearabce_prob:
                                other_acts.append(other_act[idx])

                while len(other_acts) < self.max_player_num - 1:
                    other_acts.append(-1)

                if agent_id == 0 and self.rnn_with_gnn:
                    other_acts_shuffled = [-1] * (self.max_player_num - 1)
                    other_acts_shuffled_complete = [-1] * (self.max_player_num - 1)

                    if not self.other_player_acts is None:
                        counter = 0
                        for idx, (player_locs, other_player_act) in enumerate(zip(self.prev_player_positions[1:], self.other_player_acts)):
                            counter += 1
                            if self.is_close_manhattan_dist(player_locs, self.prev_player_positions[0]) and self.prev_player_disappearance[idx+1] >= self.disappearabce_prob:
                                exact_loc = [idx_s for idx_s, idx_st in enumerate(self.prev_valid_indices) if idx_st == counter][0]
                                other_acts_shuffled[exact_loc-1] = int(other_player_act)

                        counter = 0
                        for player_locs, other_player_act in zip(self.prev_player_positions[1:], self.other_player_acts):
                            counter += 1
                            exact_loc = [
                                idx_s for idx_s, idx_st in enumerate(self.prev_valid_indices) if idx_st == counter
                            ][0]
                            other_acts_shuffled_complete[exact_loc - 1] = int(other_player_act)
                ...
                if self.collapsed:
                    ...
                    if not self.all_player_acts is None:
                        for idx, prev_acts in enumerate(self.all_player_acts):
                            ...
                            all_acts_shuffled[exact_loc] = int(prev_acts)
                ...
                if not self.collapsed and self.rnn_with_gnn:
                    observations['oppo_actions'] = oppo_actions
                    if agent_id == 0 and self.rnn_with_gnn:
                        observations['oppo_actions_shuffled'] = other_acts_shuffled
                        observations['oppo_actions_shuffled_complete'] = other_acts_shuffled_complete

            if not self.with_rgb:
                if not self.collapsed:
                    observations['num_agents'] = np.asarray([self.num_players])
                    if agent_id == 0:
                        observations['num_agents'] = np.asarray([sum([
                            self.is_close_manhattan_dist(loc, self.player_positions[0]) and self.player_disappearance[idx] >= self.disappearabce_prob
                            for idx, loc in enumerate(self.player_positions)
                        ])])
            self.prev_player_disappearance = self.player_disappearance
            return observations
    ...
    def step(self, action):
        visible_ids = [
            ...
            ) if self.is_close_manhattan_dist(self.player_positions[0], self.player_positions[idx])
            and self.prev_player_disappearance[idx] >= self.disappearabce_prob
        ]
        ...
        deleted, agents, new_types, new_types_num = self.scheduler.open_process()
        #if deleted !=[]:
        #    print("deleted, agents, new_types", deleted, agents, new_types, new_types_num)
        #print("Agents lists", self.other_agents_list[0])
        #print("Agents types", self.other_agents_types)
        ...
        mask = [
            a for a, idx in enumerate(visible_ids)
            if (not idx in deleted) and self.is_close_manhattan_dist(self.player_positions[0], self.player_positions[idx])
            and self.player_disappearance[idx] >= self.disappearabce_prob
        ]

        reverse_invisible_mask = [
            a for a, idx in enumerate(visible_ids)
            if a not in mask
        ]
        ...
        self.add_agent(agents, new_types, new_types_num)
        for idx, agent in enumerate(self.other_agents_list):
            agent.agent_id = idx + 1
        
        # print('xxxxxxxxx', self.other_agents_list)   
        ...
        remaining_flags_invisible = np.zeros(self.max_player_num) - 1
        for idx in mask:
            remaining_flags_invisible[idx] = 1
        for idx in reverse_invisible_mask:
            remaining_flags_invisible[idx] = 0

        if not self.collapsed:
            player_returns[0][0]["remaining_flags"] = remaining_flags_invisible
        ...
        teammates_list_complete = ["RandomAgent", "GreedyPredatorAgent", "GreedyProbabilisticAgent",
        ...
        res = [int(item) for item in  self.other_agents_types]
        for j in range(len(self.other_agents_list)):
            if teammates_list_complete[self.other_agents_types[j]] in str(self.other_agents_list[j]):
                pass
            else:
                print('oh no ')
                exit()
        ...
        player_returns[0][0]["teammate_types"] =  other_agents_types_full
        # player_returns[0][0]["teammates_types_nc"] = self.other_agents_list
    ...
    def sample_teammates(self, num_sampled):
        poss_teammates = [RandomAgent, GreedyPredatorAgent, GreedyProbabilisticAgent,
                          ...
                          GreedyWaitingAgent, GreedyProbabilisticWaitingAgent, 
                          TeammateAwareWaitingAgent]
        total_agents = len(poss_teammates)
        sample_type = self.prng.choice(range(len(poss_teammates)), 1)[0]
        samples = [poss_teammates[sample_type](idx+1) for idx in range(num_sampled)]
 
        return samples, sample_type

    def add_agent(self, new_agent, new_types, new_types_num=[]):
        def_orientation = 0
        ...
        self.player_disappearance.extend([self.prng.uniform() for _ in range(len(new_types))])
        self.other_agents_list.extend(new_agent)
        if new_types_num != []:
            self.other_agents_types.append(new_types_num)
    ...
    def del_agent(self, agent_id):
        for idx in agent_id:
            ...
            print(self.other_agents_types)
            self.other_agents_types.pop(idx - 1)
            print('b', self.other_agents_types)
            self.player_disappearance.pop(idx)
...
class OpenScheduler(object):
    ...
    def add_agents(self, agent_nums):
        ...
        for _ in range(agent_nums):
            agent_class, agent_id_type = self.agent_type_sampler()
        ...
        return new_agent, new_obs_type, agent_id_type
    ...
    def agent_type_sampler(self):
        ...
        return all_agent_types[agent_id_type], agent_id_type
    ...
    def open_process(self):
        ...
        new_agents, new_obs_type, new_agent_type_num = [], [], []
        if add:
            agent_nums = min(self.prng.choice(2, 1, p=[0.7, 0.3])[0] + 1,
                                self.max_available_agents - self.available_agents)
            new_agents, new_obs_type, new_agent_type_num = self.add_agents(agent_nums)
        return deleted_idxs, new_agents, new_obs_type, new_agent_type_num
