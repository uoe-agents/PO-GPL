...
class ForagingEnv(Env):
    ...
    def _get_observation_space_real(self):
        ...
        if not self.with_gnn_shuffle:
            return gym.spaces.Dict({
                ...
                'food_info_complete': gym.spaces.Box(np.array(min_food_obs), np.array(max_food_obs), dtype=np.int32),
    ...
    def reset(self):
        ...
        for i in range(len(self.players)):
            ...
            player_obs[3 * i + 1] = -1
            player_obs[3 * i + 2] = 0
            complete_player_obs[3 * i] = -1
            complete_player_obs[3 * i + 1] = -1
            complete_player_obs[3 * i + 2] = 0
        ...
        for idx_write in range(len(valid_elements)):
            player_obs[3 * idx_write: 3 * idx_write + 3] = \
                nobs[0][1][
                3 * self.max_food + 3 * element_shuffled_indices[idx_write]:3 * self.max_food + 3 * element_shuffled_indices[idx_write] + 3]
        ...
        for idx_write in range(len(complete_valid_elements)):
            complete_player_obs[3 * idx_write: 3 * idx_write + 3] = \
                n_obs_complete[0][1][
                    3 * self.max_food + 3 * complete_element_shuffled_indices[idx_write]:3 * self.max_food + 3 * complete_element_shuffled_indices[idx_write] + 3
                ]

        if self.with_gnn_shuffle:
            player_obs_shuffled = np.zeros(len(self.players) * 3, np.int32)
            complete_player_obs_shuffled = np.zeros(len(self.players) * 3, np.int32)

            for i in range(len(self.players)):
                player_obs_shuffled[3 * i] = -1
                player_obs_shuffled[3 * i + 1] = -1
                player_obs_shuffled[3 * i + 2] = 0
                complete_player_obs_shuffled[3 * i] = -1
                complete_player_obs_shuffled[3 * i + 1] = -1
                complete_player_obs_shuffled[3 * i + 2] = 0
            ...
            for i in range(len(complete_non_zero_idxes)):
                complete_player_obs_shuffled[3 * complete_non_zero_idxes[i]: 3 * complete_non_zero_idxes[i] + 3] = \
                   n_obs_complete[0][1][self.max_food * 3 + 3 * i:self.max_food * 3 + 3 * i + 3]

        food_info_obs = nobs[0][1][:3 * self.max_food].astype(np.int32)
        food_info_complete = n_obs_complete[0][1][:3 * self.max_food].astype(np.int32)
        ...
        if not self.with_gnn_shuffle:
            return {
                ...
                "food_info_complete": food_info_complete,
    ...
    def step(self, action):
        ...
        for idx_write in range(len(valid_elements)):
            player_obs[3 * idx_write: 3 * idx_write + 3] = \
                nobs[0][1][
                3 * self.max_food + 3 * element_shuffled_indices[idx_write]:3 * self.max_food + 3 *
                                                                            element_shuffled_indices[idx_write] + 3]

        for idx_write in range(len(complete_valid_elements)):
            complete_player_obs[3 * idx_write: 3 * idx_write + 3] = \
                n_obs_complete[0][1][
                3 * self.max_food + 3 * complete_element_shuffled_indices[idx_write]:3 * self.max_food + 3 *
                                                                            complete_element_shuffled_indices[idx_write] + 3]
        ...
        food_info_obs = nobs[0][1][:3 * self.max_food].astype(np.int32)
        complete_food_info_obs = n_obs_complete[0][1][:3 * self.max_food].astype(np.int32)
        player_info = player_obs.astype(np.int32)
        complete_player_info = complete_player_obs.astype(np.int32)
        remaining_info = np.array(self.remaining_idxs, np.int32)
        complete_remaining_info = np.array(self.complete_remaining_idxs, np.int32)
