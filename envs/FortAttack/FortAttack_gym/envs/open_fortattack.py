...
from FortAttack_gym.envs.multiagent.multi_discrete import MultiDiscrete
from FortAttack_gym.envs.malib.spaces import Box
from FortAttack_gym.envs.learner import Learner
from FortAttack_gym.envs.mpnn import MPNN
from FortAttack_gym.envs.rlagent import Neo
import torch
from argparse import Namespace
from FortAttack_gym.envs.core import World, Agent
import numpy as np
...
class OpenFortAttackGlobalEnv(gym.Env):
    ...
    def postprocess_obs(self, obs, acts=None):
        in_distance = np.sqrt(np.sum((obs[:, 1:3] - obs[0, 1:3]) ** 2, axis=-1)) < self.vision_radius
        available = obs[:, 0] == 1
        if self.obs_mode != "conic":
            is_valid = np.logical_and(in_distance, available)
        else:
            delta_x = obs[1:,1] - obs[0,1]
            delta_y = obs[1:,2] - obs[0,2]

            teammate_angles = (np.arctan2(delta_y, delta_x) + (2*np.pi)) % (2*np.pi)
            agent_angle = obs[0,3]

            angle_limits_up = ((teammate_angles - agent_angle) % (2*np.pi)) <= self.cone_angle/2
            angle_limits_down = ((agent_angle - teammate_angles) % (2*np.pi)) <= self.cone_angle/2

            in_angle = np.logical_or(angle_limits_up, angle_limits_down)
            in_angle_updated = np.concatenate([np.array([True]),in_angle], axis=-1)
            is_valid = np.logical_and(in_angle_updated, in_distance)
            is_valid = np.logical_and(is_valid, available)

        removed = np.copy(obs)
        removed[np.logical_not(is_valid), 1:] = -1
        removed[np.logical_not(is_valid), 0] = 0

        # Create default not exist prev obs
        if self.ne_prev_obs is None:
            self.ne_prev_obs = np.zeros([obs.shape[0], 1])[:, 0] != 1

        if not acts is None:
            act_copy = np.copy(acts)
            act_copy[self.ne_prev_obs, :] = -1
            self.ne_prev_obs = np.logical_not(is_valid)

            return removed, act_copy
        return removed

    def __init__(self, max_timesteps=100, num_guards=5, num_attackers=5, active_agents=3, num_freeze_steps=20,
                 reward_mode="normal", obs_mode="conic", cone_angle=np.pi/3,
                 shared_viewer=True, arguments = {}, vision_radius=2.0,
                 seed=100, with_oppo_modelling=False, team_mode=None, agent_type=-1, with_cuda=True):

        scenario = FortAttackEnvV2(
            num_guards=num_guards, num_attackers=num_attackers,
            active_agents=active_agents, num_freeze_steps=num_freeze_steps,
        ...
        self.world.max_time_steps = max_timesteps
        self.seed_val = seed
        ...
        self.seed_callback = scenario.seed
        self.policy_id = None
        ...
        self.observation_space = spaces.Dict({
            "player_obs" : Box(low=-np.inf, high=+np.inf, shape=(self.world.numAgents, obs_shapes[0][0]+2)),
            "complete_player_obs" : Box(low=-np.inf, high=+np.inf, shape=(self.world.numAgents, obs_shapes[0][0]+2))
        })
        ...
        if self.with_oppo_modelling:
            self.observation_space = spaces.Dict({
                "player_obs" : Box(low=-np.inf, high=+np.inf, shape=(self.world.numAgents, obs_shapes[0][0] + 2)),
                "complete_player_obs" : Box(low=-np.inf, high=+np.inf, shape=(self.world.numAgents, obs_shapes[0][0] + 2)),
                "prev_actions" : Box(low=-np.inf, high=+np.inf, shape=(self.world.numAgents, 1))
            })
    ...
    def step(self, action_n):
        ...
        concat_obs_n[concat_obs_n[:,0]!=1,1:] = -1
        ...
        if self.with_oppo_modelling:
            #updated_obs, acts = self.postprocess_obs(concat_obs_n, swapped_prev_actions)
            return {
                       "complete_player_obs": concat_obs_n,
                       "player_obs": concat_obs_n,
                       "prev_actions": swapped_prev_actions
                   }, 0.1*reward_n[self.ad_hoc_agent_id], done, info_n['n'][self.ad_hoc_agent_id]
    ...
    def reset(self):
        ...
        for id, agent in enumerate(self.world.all_agents):
            ...
            agent_pol_dir = os.path.join(dirname, prefix + controller_idx)
            if not agent.attacker:
        ...
        if self.with_oppo_modelling:
            ...
            return {
                "complete_player_obs" : concat_obs_n,
                "player_obs" : updated_obs,
                "prev_actions" : acts
            }
        return {
            "complete_player_obs" : concat_obs_n,
            "player_obs": self.postprocess_obs(concat_obs_n, None)
        }
...
class FortAttackEnvV2:
    ...
    def observation(self, agent, world):
        ...
        output = (np.concatenate([[agent.alive]] + [agent.state.p_pos] + orien + [agent.state.p_vel] + entity_pos))
