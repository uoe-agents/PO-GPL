...
class MARLNavigationEnv(Env):
    ...
    action_set = [Action.NORTH, Action.SOUTH, Action.WEST, Action.EAST, Action.NOTHING]
    ...
    def action_space(self):
        return [gym.spaces.Discrete(6) for _ in range(len(self.players))]
    ...
    def _get_observation_space(self):
        ...
        min_obs = [-1, -1] * max_food + [-1, -1] * len(self.players) + [-1]
    ...
    def spawn_food(self, max_food):
        ...
        while food_count < max_food and attempts < 2500:
            ...
            if (
                    self.neighborhood(row, col, distance=3).sum() > 0
    ...
    def step(self, action):
        ...
        dest_pos_row, dest_pos_col = np.nonzero(self.field)
        for player in self.active_players:
            prow, pcol = player.position
            min_dist = min([abs(prow-dest_row)+abs(pcol-dest_col) for dest_row, dest_col in zip(dest_pos_row, dest_pos_col)])
            player.reward += -0.025 * min_dist
        ...
        return np.asarray([a[1] for a in nobs]), nreward, all([ndone for _ in range(len(self.players))]), ninfo
